...
public class ChatAdapter extends BaseAdapter {
    ...
    public static final String MSG_SENT = "0";
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(isOutgoing && recipientId != null )
        {
            if(chatMessage.getReadIds() != null && (chatMessage.getReadIds().contains(recipientId)))
            {
                holder.txtStatus.setVisibility(View.VISIBLE);
                holder.txtStatus.setText("Read");
            }
            else if(chatMessage.getDeliveredIds() != null && chatMessage.getDeliveredIds().contains(recipientId))
            {
                holder.txtStatus.setVisibility(View.VISIBLE);
    ...
    private ViewHolder createViewHolder(View v) {
        ...
        holder.txtStatus = (TextView) v.findViewById(R.id.tv_status);
        return holder;
    }
    ...
    public void updateReadStatus(QBChatMessage qbChatMessage)
    {
        int index = chatMessages.indexOf(qbChatMessage);
        if (index != -1) {
            QBChatMessage qbChatMsg = chatMessages.get(index);
            Collection<Integer> readIds = qbChatMsg.getReadIds();
            if(readIds != null)
            {
                readIds.add(qbChatMessage.getRecipientId());
            }else
            {
                readIds = new ArrayList<Integer>();
                readIds.add(qbChatMessage.getRecipientId());
                qbChatMsg.setReadIds(readIds);
            }
            notifyDataSetChanged();
        }
    }

    public void updateDeliveredStatus(QBChatMessage qbChatMessage)
    {
        int index = chatMessages.indexOf(qbChatMessage);
        if (index != -1) {
            QBChatMessage qbChatMsg = chatMessages.get(index);
            Collection<Integer> deliveredIds = qbChatMsg.getDeliveredIds();
            if(deliveredIds != null)
            {
                deliveredIds.add(qbChatMessage.getRecipientId());
            }else
            {
                deliveredIds = new ArrayList<Integer>();
                deliveredIds.add(qbChatMessage.getRecipientId());
                qbChatMsg.setDeliveredIds(deliveredIds);
            }
